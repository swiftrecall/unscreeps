...
export class CreepSpawnRequest implements Partial<ICreepSpawnRequest> {
	...
	constructor({ serialized, type, priority, body }: Partial<ICreepSpawnRequest> & { serialized?: string } = {}) {
		if (serialized) {
...
export class SpawnRequestQueue {
	...
	static deserialize(memoryStore: string): SpawnRequestQueue {
		return !memoryStore ? new SpawnRequestQueue() : new SpawnRequestQueue().deserialize(memoryStore);
	}
	...
	public serialize(): string {
		return this.queue
			.map<string>((spawnRequest: CreepSpawnRequest) => spawnRequest.serialize())
			.join('|');
	}

	public deserialize(_serialized: string): this {
		this.queue = ((_serialized && _serialized.split('|')) || []).map((serializedRequest: string) => new CreepSpawnRequest({ serialized: serializedRequest }));
		return this;
	}
